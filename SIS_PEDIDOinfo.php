...
//
// Table class for SIS_PEDIDO
//
class cSIS_PEDIDO extends cTable {
	var $numpedido;
	var $codcliente;
	var $fechaultimamod;
	...
	function __construct() {
		...
		$this->TableVar = 'SIS_PEDIDO';
		$this->TableName = 'SIS_PEDIDO';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "[dbo].[SIS_PEDIDO]";
		$this->DBID = 'DB';
		...
		// numpedido
		$this->numpedido = new cField('SIS_PEDIDO', 'SIS_PEDIDO', 'x_numpedido', 'numpedido', '[numpedido]', 'CAST([numpedido] AS NVARCHAR)', 3, -1, FALSE, '[numpedido]', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->numpedido->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['numpedido'] = &$this->numpedido;

		// codcliente
		$this->codcliente = new cField('SIS_PEDIDO', 'SIS_PEDIDO', 'x_codcliente', 'codcliente', '[codcliente]', 'CAST([codcliente] AS NVARCHAR)', 3, -1, FALSE, '[EV__codcliente]', TRUE, TRUE, TRUE, 'FORMATTED TEXT', 'SELECT');
		$this->codcliente->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['codcliente'] = &$this->codcliente;

		// fechaultimamod
		$this->fechaultimamod = new cField('SIS_PEDIDO', 'SIS_PEDIDO', 'x_fechaultimamod', 'fechaultimamod', '[fechaultimamod]', '[fechaultimamod]', 202, -1, FALSE, '[fechaultimamod]', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['fechaultimamod'] = &$this->fechaultimamod;

		// usuarioultimamod
		$this->usuarioultimamod = new cField('SIS_PEDIDO', 'SIS_PEDIDO', 'x_usuarioultimamod', 'usuarioultimamod', '[usuarioultimamod]', '[usuarioultimamod]', 200, -1, FALSE, '[usuarioultimamod]', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['usuarioultimamod'] = &$this->usuarioultimamod;

		// fecha
		$this->fecha = new cField('SIS_PEDIDO', 'SIS_PEDIDO', 'x_fecha', 'fecha', '[fecha]', '[fecha]', 202, -1, FALSE, '[fecha]', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['fecha'] = &$this->fecha;

		// descuento
		$this->descuento = new cField('SIS_PEDIDO', 'SIS_PEDIDO', 'x_descuento', 'descuento', '[descuento]', 'CAST([descuento] AS NVARCHAR)', 5, -1, FALSE, '[descuento]', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// estado
		$this->estado = new cField('SIS_PEDIDO', 'SIS_PEDIDO', 'x_estado', 'estado', '[estado]', '[estado]', 200, -1, FALSE, '[estado]', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'HIDDEN');
		$this->fields['estado'] = &$this->estado;

		// transporte
		$this->transporte = new cField('SIS_PEDIDO', 'SIS_PEDIDO', 'x_transporte', 'transporte', '[transporte]', 'CAST([transporte] AS NVARCHAR)', 5, -1, FALSE, '[transporte]', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function GetDetailUrl() {
		...
		// Detail url
		$sDetailUrl = "";
		if ($this->getCurrentDetailTable() == "SIS_PEDIDO_DETALLE") {
			$sDetailUrl = $GLOBALS["SIS_PEDIDO_DETALLE"]->GetListUrl() . "?" . EW_TABLE_SHOW_MASTER . "=" . $this->TableVar;
			$sDetailUrl .= "&fk_numpedido=" . urlencode($this->numpedido->CurrentValue);
		}
		if ($sDetailUrl == "") {
			$sDetailUrl = "SIS_PEDIDOlist.php";
		}
		return $sDetailUrl;
	}
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "[dbo].[SIS_PEDIDO]";
	}
	...
	function getSqlSelectList() { // Select for List page
		$select = "";
		$select = "SELECT * FROM (" .
			"SELECT *, (SELECT TOP 1 [nombre] FROM [dbo].[SIS_CLIENTE] [EW_TMP_LOOKUPTABLE] WHERE [EW_TMP_LOOKUPTABLE].[codcliente] = [SIS_PEDIDO].[codcliente]) AS [EV__codcliente] FROM [dbo].[SIS_PEDIDO]" .
			") [EW_TMP_TABLE]";
	...
	function getSqlWhere() { // Where
		...
		$this->TableFilter = "estado = 'PENDIENTE'";
	...
	function UseVirtualFields() {
		...
		if ($this->codcliente->AdvancedSearch->SearchValue <> "" ||
			$this->codcliente->AdvancedSearch->SearchValue2 <> "" ||
			strpos($sWhere, " " . $this->codcliente->FldVirtualExpression . " ") !== FALSE)
			return TRUE;
		if (strpos($sOrderBy, " " . $this->codcliente->FldVirtualExpression . " ") !== FALSE)
			return TRUE;
		return FALSE;
	}
	...
	function Update(&$rs, $where = "", $rsold = NULL, $curfilter = TRUE) {
		...
		// Cascade Update detail table 'SIS_PEDIDO_DETALLE'
		$bCascadeUpdate = FALSE;
		$rscascade = array();
		if (!is_null($rsold) && (isset($rs['numpedido']) && $rsold['numpedido'] <> $rs['numpedido'])) { // Update detail field 'numpedido'
			$bCascadeUpdate = TRUE;
			$rscascade['numpedido'] = $rs['numpedido']; 
		}
		if ($bCascadeUpdate) {
			if (!isset($GLOBALS["SIS_PEDIDO_DETALLE"])) $GLOBALS["SIS_PEDIDO_DETALLE"] = new cSIS_PEDIDO_DETALLE();
			$rswrk = $GLOBALS["SIS_PEDIDO_DETALLE"]->LoadRs("[numpedido] = " . ew_QuotedValue($rsold['numpedido'], EW_DATATYPE_NUMBER, 'DB')); 
			while ($rswrk && !$rswrk->EOF) {
				$GLOBALS["SIS_PEDIDO_DETALLE"]->Update($rscascade, "[numpedido] = " . ew_QuotedValue($rsold['numpedido'], EW_DATATYPE_NUMBER, 'DB'), $rswrk->fields);
				$rswrk->MoveNext();
			}
		}
	...
	function Delete(&$rs, $where = "", $curfilter = TRUE) {
		...
		// Cascade delete detail table 'SIS_PEDIDO_DETALLE'
		if (!isset($GLOBALS["SIS_PEDIDO_DETALLE"])) $GLOBALS["SIS_PEDIDO_DETALLE"] = new cSIS_PEDIDO_DETALLE();
		$rscascade = $GLOBALS["SIS_PEDIDO_DETALLE"]->LoadRs("[numpedido] = " . ew_QuotedValue($rs['numpedido'], EW_DATATYPE_NUMBER, "DB")); 
		while ($rscascade && !$rscascade->EOF) {
			$GLOBALS["SIS_PEDIDO_DETALLE"]->Delete($rscascade->fields);
			$rscascade->MoveNext();
		}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("SIS_PEDIDOview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("SIS_PEDIDOview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "SIS_PEDIDOadd.php?" . $this->UrlParm($parm);
		else
			$url = "SIS_PEDIDOadd.php";
		return $this->AddMasterUrl($url);
	}
	...
	function GetEditUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("SIS_PEDIDOedit.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("SIS_PEDIDOedit.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("SIS_PEDIDOadd.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("SIS_PEDIDOadd.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("SIS_PEDIDOdelete.php", $this->UrlParm());
	}
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			if ($isPost && isset($_POST["numpedido"]))
				$arKeys[] = ew_StripSlashes($_POST["numpedido"]);
			elseif (isset($_GET["numpedido"]))
				$arKeys[] = ew_StripSlashes($_GET["numpedido"]);
			else
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->numpedido->CurrentValue = $key;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->fechaultimamod->Exportable) $Doc->ExportCaption($this->fechaultimamod);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->fechaultimamod->Exportable) $Doc->ExportField($this->fechaultimamod);
