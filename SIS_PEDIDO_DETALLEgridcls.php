...
$SIS_PEDIDO_DETALLE_grid = NULL; // Initialize page object first

class cSIS_PEDIDO_DETALLE_grid extends cSIS_PEDIDO_DETALLE {
	...
	// Page object name
	var $PageObjName = 'SIS_PEDIDO_DETALLE_grid';

	// Grid form hidden field names
	var $FormName = 'fSIS_PEDIDO_DETALLEgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["SIS_PEDIDO_DETALLE"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->numpedido->CurrentValue;
					if ($sKey <> "") $sKey .= $GLOBALS["EW_COMPOSITE_KEY_SEPARATOR"];
					$sKey .= $this->numdetpedido->CurrentValue;
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_numpedido") && $objForm->HasValue("o_numpedido") && $this->numpedido->CurrentValue <> $this->numpedido->OldValue)
			return FALSE;
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->numpedido->CurrentValue . $GLOBALS["EW_COMPOSITE_KEY_SEPARATOR"] . $this->numdetpedido->CurrentValue . "\">";
		}
	...
	function SetRecordKey(&$key, $rs) {
		...
		$key .= $rs->fields('numpedido');
		if ($key <> "") $key .= $GLOBALS["EW_COMPOSITE_KEY_SEPARATOR"];
		$key .= $rs->fields('numdetpedido');
	}
	...
	function LoadDefaultValues() {
		...
		$this->numdetpedido->CurrentValue = NULL;
		$this->numdetpedido->OldValue = $this->numdetpedido->CurrentValue;
	...
	function LoadFormValues() {
		...
		$this->numpedido->setOldValue($objForm->GetValue("o_numpedido"));
		if (!$this->numdetpedido->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 2) {
			if (strval($arKeys[0]) <> "")
				$this->numpedido->CurrentValue = strval($arKeys[0]); // numpedido
			else
				$bValidKey = FALSE;
			if (strval($arKeys[1]) <> "")
				$this->numdetpedido->CurrentValue = strval($arKeys[1]); // numdetpedido
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
