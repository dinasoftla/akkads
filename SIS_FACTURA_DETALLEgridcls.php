...
$SIS_FACTURA_DETALLE_grid = NULL; // Initialize page object first

class cSIS_FACTURA_DETALLE_grid extends cSIS_FACTURA_DETALLE {
	...
	// Page object name
	var $PageObjName = 'SIS_FACTURA_DETALLE_grid';

	// Grid form hidden field names
	var $FormName = 'fSIS_FACTURA_DETALLEgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["SIS_FACTURA_DETALLE"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					if ($sKey <> "") $sKey .= $GLOBALS["EW_COMPOSITE_KEY_SEPARATOR"];
					$sKey .= $this->numdetfactura->CurrentValue;
	...
	function EmptyRow() {
		...
		if ($objForm->HasValue("x_numfactura") && $objForm->HasValue("o_numfactura") && $this->numfactura->CurrentValue <> $this->numfactura->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_codarticulo") && $objForm->HasValue("o_codarticulo") && $this->codarticulo->CurrentValue <> $this->codarticulo->OldValue)
			return FALSE;
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->numfactura->CurrentValue . $GLOBALS["EW_COMPOSITE_KEY_SEPARATOR"] . $this->numdetfactura->CurrentValue . "\">";
		}
	...
	function LoadDefaultValues() {
		...
		$this->numdetfactura->CurrentValue = NULL;
		$this->numdetfactura->OldValue = $this->numdetfactura->CurrentValue;
	...
	function LoadFormValues() {
		...
		if (!$this->numdetfactura->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
		...
		$this->codarticulo->setOldValue($objForm->GetValue("o_codarticulo"));
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 2) {
			if (strval($arKeys[0]) <> "")
				...
				$bValidKey = FALSE;
			if (strval($arKeys[1]) <> "")
				$this->numdetfactura->CurrentValue = strval($arKeys[1]); // numdetfactura
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "SIS_FACTURA") {
			$this->numfactura->Visible = FALSE;
			if ($GLOBALS["SIS_FACTURA"]->EventCancelled) $this->EventCancelled = TRUE;
		}
