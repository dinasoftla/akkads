...
//
// Table class for SIS_CLIENTE
//
class cSIS_CLIENTE extends cTable {
	var $codcliente;
	var $nombre;
	var $telefonos;
	var $_email;
	...
	function __construct() {
		...
		$this->TableVar = 'SIS_CLIENTE';
		$this->TableName = 'SIS_CLIENTE';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "[dbo].[SIS_CLIENTE]";
		$this->DBID = 'DB';
		...
		// codcliente
		$this->codcliente = new cField('SIS_CLIENTE', 'SIS_CLIENTE', 'x_codcliente', 'codcliente', '[codcliente]', 'CAST([codcliente] AS NVARCHAR)', 3, -1, FALSE, '[codcliente]', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->codcliente->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['codcliente'] = &$this->codcliente;

		// nombre
		$this->nombre = new cField('SIS_CLIENTE', 'SIS_CLIENTE', 'x_nombre', 'nombre', '[nombre]', '[nombre]', 200, -1, FALSE, '[nombre]', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['nombre'] = &$this->nombre;

		// telefonos
		$this->telefonos = new cField('SIS_CLIENTE', 'SIS_CLIENTE', 'x_telefonos', 'telefonos', '[telefonos]', '[telefonos]', 200, -1, FALSE, '[telefonos]', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['telefonos'] = &$this->telefonos;

		// email
		$this->_email = new cField('SIS_CLIENTE', 'SIS_CLIENTE', 'x__email', 'email', '[email]', '[email]', 200, -1, FALSE, '[email]', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['email'] = &$this->_email;

		// direccion
		$this->direccion = new cField('SIS_CLIENTE', 'SIS_CLIENTE', 'x_direccion', 'direccion', '[direccion]', '[direccion]', 200, -1, FALSE, '[direccion]', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['direccion'] = &$this->direccion;

		// tipoprecio
		$this->tipoprecio = new cField('SIS_CLIENTE', 'SIS_CLIENTE', 'x_tipoprecio', 'tipoprecio', '[tipoprecio]', '[tipoprecio]', 129, -1, FALSE, '[tipoprecio]', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		$this->tipoprecio->OptionCount = 2;
		$this->fields['tipoprecio'] = &$this->tipoprecio;

		// fecultimamod
		$this->fecultimamod = new cField('SIS_CLIENTE', 'SIS_CLIENTE', 'x_fecultimamod', 'fecultimamod', '[fecultimamod]', '[fecultimamod]', 202, -1, FALSE, '[fecultimamod]', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['fecultimamod'] = &$this->fecultimamod;

		// usuarioultimamod
		$this->usuarioultimamod = new cField('SIS_CLIENTE', 'SIS_CLIENTE', 'x_usuarioultimamod', 'usuarioultimamod', '[usuarioultimamod]', '[usuarioultimamod]', 200, -1, FALSE, '[usuarioultimamod]', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['usuarioultimamod'] = &$this->usuarioultimamod;

		// tipo
		$this->tipo = new cField('SIS_CLIENTE', 'SIS_CLIENTE', 'x_tipo', 'tipo', '[tipo]', '[tipo]', 200, -1, FALSE, '[tipo]', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "[dbo].[SIS_CLIENTE]";
	}
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('codcliente', $rs))
				ew_AddFilter($where, ew_QuotedName('codcliente', $this->DBID) . '=' . ew_QuotedValue($rs['codcliente'], $this->codcliente->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		if (!is_numeric($this->codcliente->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@codcliente@", ew_AdjustSql($this->codcliente->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("SIS_CLIENTEview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("SIS_CLIENTEview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "SIS_CLIENTEadd.php?" . $this->UrlParm($parm);
		else
			$url = "SIS_CLIENTEadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("SIS_CLIENTEedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("SIS_CLIENTEadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("SIS_CLIENTEdelete.php", $this->UrlParm());
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "codcliente:" . ew_VarToJson($this->codcliente->CurrentValue, "number", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->codcliente->CurrentValue)) {
			$sUrl .= "codcliente=" . urlencode($this->codcliente->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			if ($isPost && isset($_POST["codcliente"]))
				$arKeys[] = ew_StripSlashes($_POST["codcliente"]);
			elseif (isset($_GET["codcliente"]))
				$arKeys[] = ew_StripSlashes($_GET["codcliente"]);
			else
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->codcliente->CurrentValue = $key;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->tipoprecio->Exportable) $Doc->ExportCaption($this->tipoprecio);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->tipoprecio->Exportable) $Doc->ExportField($this->tipoprecio);
