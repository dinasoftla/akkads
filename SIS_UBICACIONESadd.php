...
$SIS_UBICACIONES_add = NULL; // Initialize page object first

class cSIS_UBICACIONES_add extends cSIS_UBICACIONES {
	...
	// Page object name
	var $PageObjName = 'SIS_UBICACIONES_add';
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["codubicacion"] != "") {
				...
				$this->setKey("codubicacion", $this->codubicacion->CurrentValue); // Set up key
			} else {
				$this->setKey("codubicacion", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "SIS_UBICACIONESview.php")
...
// Create page object
if (!isset($SIS_UBICACIONES_add)) $SIS_UBICACIONES_add = new cSIS_UBICACIONES_add();

// Page init
$SIS_UBICACIONES_add->Page_Init();

// Page main
$SIS_UBICACIONES_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$SIS_UBICACIONES_add->Page_Render();
?>
...
var CurrentForm = fSIS_UBICACIONESadd = new ew_Form("fSIS_UBICACIONESadd", "add");

// Validate form
fSIS_UBICACIONESadd.Validate = function() {
...
fSIS_UBICACIONESadd.Form_CustomValidate = 
...
fSIS_UBICACIONESadd.ValidateRequired = true;
<?php } else { ?>
fSIS_UBICACIONESadd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fSIS_UBICACIONESadd.Lists["x_imprimir[]"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fSIS_UBICACIONESadd.Lists["x_imprimir[]"].Options = <?php echo json_encode($SIS_UBICACIONES->imprimir->Options()) ?>;
...
<?php $SIS_UBICACIONES_add->ShowPageHeader(); ?>
<?php
$SIS_UBICACIONES_add->ShowMessage();
?>
<form name="fSIS_UBICACIONESadd" id="fSIS_UBICACIONESadd" class="<?php echo $SIS_UBICACIONES_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($SIS_UBICACIONES_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $SIS_UBICACIONES_add->Token ?>">
<?php } ?>
...
<?php if ($SIS_UBICACIONES->ubicacion->Visible) { // ubicacion ?>
	<div id="r_ubicacion" class="form-group">
		<label id="elh_SIS_UBICACIONES_ubicacion" for="x_ubicacion" class="col-sm-2 control-label ewLabel"><?php echo $SIS_UBICACIONES->ubicacion->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $SIS_UBICACIONES->ubicacion->CellAttributes() ?>>
<span id="el_SIS_UBICACIONES_ubicacion">
<input type="text" data-table="SIS_UBICACIONES" data-field="x_ubicacion" name="x_ubicacion" id="x_ubicacion" size="30" maxlength="12" placeholder="<?php echo ew_HtmlEncode($SIS_UBICACIONES->ubicacion->getPlaceHolder()) ?>" value="<?php echo $SIS_UBICACIONES->ubicacion->EditValue ?>"<?php echo $SIS_UBICACIONES->ubicacion->EditAttributes() ?>>
</span>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $SIS_UBICACIONES_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
</form>
<script type="text/javascript">
fSIS_UBICACIONESadd.Init();
</script>
<?php
$SIS_UBICACIONES_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$SIS_UBICACIONES_add->Page_Terminate();
?>
