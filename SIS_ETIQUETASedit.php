...
$SIS_ETIQUETAS_edit = NULL; // Initialize page object first

class cSIS_ETIQUETAS_edit extends cSIS_ETIQUETAS {
	...
	// Page object name
	var $PageObjName = 'SIS_ETIQUETAS_edit';
	...
	function Page_Main() {
		...
		// Check if valid key
		if ($this->idetiqueta->CurrentValue == "")
			$this->Page_Terminate("SIS_ETIQUETASlist.php"); // Invalid key, return to list
	...
	function LoadFormValues() {
		...
		if (!$this->idetiqueta->FldIsDetailKey)
			$this->idetiqueta->setFormValue($objForm->GetValue("x_idetiqueta"));
	...
	function RestoreFormValues() {
		...
		$this->idetiqueta->CurrentValue = $this->idetiqueta->FormValue;
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// referenciaasignada
			$this->referenciaasignada->SetDbValueDef($rsnew, $this->referenciaasignada->CurrentValue, "", $this->referenciaasignada->ReadOnly);

			// nomusuario
			$this->nomusuario->SetDbValueDef($rsnew, $this->nomusuario->CurrentValue, "", $this->nomusuario->ReadOnly);
...
// Create page object
if (!isset($SIS_ETIQUETAS_edit)) $SIS_ETIQUETAS_edit = new cSIS_ETIQUETAS_edit();

// Page init
$SIS_ETIQUETAS_edit->Page_Init();

// Page main
$SIS_ETIQUETAS_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$SIS_ETIQUETAS_edit->Page_Render();
?>
...
var CurrentForm = fSIS_ETIQUETASedit = new ew_Form("fSIS_ETIQUETASedit", "edit");

// Validate form
fSIS_ETIQUETASedit.Validate = function() {
...
fSIS_ETIQUETASedit.Form_CustomValidate = 
...
fSIS_ETIQUETASedit.ValidateRequired = true;
<?php } else { ?>
fSIS_ETIQUETASedit.ValidateRequired = false; 
<?php } ?>
...
<?php $SIS_ETIQUETAS_edit->ShowPageHeader(); ?>
<?php
$SIS_ETIQUETAS_edit->ShowMessage();
?>
<form name="fSIS_ETIQUETASedit" id="fSIS_ETIQUETASedit" class="<?php echo $SIS_ETIQUETAS_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($SIS_ETIQUETAS_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $SIS_ETIQUETAS_edit->Token ?>">
<?php } ?>
...
<?php if ($SIS_ETIQUETAS->idetiqueta->Visible) { // idetiqueta ?>
	<div id="r_idetiqueta" class="form-group">
		<label id="elh_SIS_ETIQUETAS_idetiqueta" class="col-sm-2 control-label ewLabel"><?php echo $SIS_ETIQUETAS->idetiqueta->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $SIS_ETIQUETAS->idetiqueta->CellAttributes() ?>>
<span id="el_SIS_ETIQUETAS_idetiqueta">
<span<?php echo $SIS_ETIQUETAS->idetiqueta->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $SIS_ETIQUETAS->idetiqueta->EditValue ?></p></span>
</span>
<input type="hidden" data-table="SIS_ETIQUETAS" data-field="x_idetiqueta" name="x_idetiqueta" id="x_idetiqueta" value="<?php echo ew_HtmlEncode($SIS_ETIQUETAS->idetiqueta->CurrentValue) ?>">
<?php echo $SIS_ETIQUETAS->idetiqueta->CustomMsg ?></div></div>
	</div>
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $SIS_ETIQUETAS_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
</form>
<script type="text/javascript">
fSIS_ETIQUETASedit.Init();
</script>
<?php
$SIS_ETIQUETAS_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$SIS_ETIQUETAS_edit->Page_Terminate();
?>
