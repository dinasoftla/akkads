...
$SIS_TAREAS_add = NULL; // Initialize page object first

class cSIS_TAREAS_add extends cSIS_TAREAS {
	...
	// Page object name
	var $PageObjName = 'SIS_TAREAS_add';
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["codtarea"] != "") {
				...
				$this->setKey("codtarea", $this->codtarea->CurrentValue); // Set up key
			} else {
				$this->setKey("codtarea", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "SIS_TAREASview.php")
	...
	function LoadDefaultValues() {
		...
		$this->respondable->CurrentValue = NULL;
		$this->respondable->OldValue = $this->respondable->CurrentValue;
	...
	function AddRow($rsold = NULL) {
		...
		// respondable
		$this->respondable->SetDbValueDef($rsnew, $this->respondable->CurrentValue, NULL, FALSE);

		// fechainicio
		$this->fechainicio->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->fechainicio->CurrentValue, 7), NULL, FALSE);

		// fechafinal
		$this->fechafinal->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->fechafinal->CurrentValue, 7), NULL, FALSE);
		...
		if ($bInsertRow) {
			...
			if ($AddRow) {
				...
				$this->codtarea->setDbValue($conn->Insert_ID());
				$rsnew['codtarea'] = $this->codtarea->DbValue;
			}
		} else {
...
// Create page object
if (!isset($SIS_TAREAS_add)) $SIS_TAREAS_add = new cSIS_TAREAS_add();

// Page init
$SIS_TAREAS_add->Page_Init();

// Page main
$SIS_TAREAS_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$SIS_TAREAS_add->Page_Render();
?>
...
var CurrentForm = fSIS_TAREASadd = new ew_Form("fSIS_TAREASadd", "add");

// Validate form
fSIS_TAREASadd.Validate = function() {
...
fSIS_TAREASadd.Form_CustomValidate = 
...
fSIS_TAREASadd.ValidateRequired = true;
<?php } else { ?>
fSIS_TAREASadd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fSIS_TAREASadd.Lists["x_estado"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fSIS_TAREASadd.Lists["x_estado"].Options = <?php echo json_encode($SIS_TAREAS->estado->Options()) ?>;
fSIS_TAREASadd.Lists["x_respondable"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fSIS_TAREASadd.Lists["x_respondable"].Options = <?php echo json_encode($SIS_TAREAS->respondable->Options()) ?>;
fSIS_TAREASadd.Lists["x_prioridad"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fSIS_TAREASadd.Lists["x_prioridad"].Options = <?php echo json_encode($SIS_TAREAS->prioridad->Options()) ?>;
...
<?php $SIS_TAREAS_add->ShowPageHeader(); ?>
<?php
$SIS_TAREAS_add->ShowMessage();
?>
<form name="fSIS_TAREASadd" id="fSIS_TAREASadd" class="<?php echo $SIS_TAREAS_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($SIS_TAREAS_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $SIS_TAREAS_add->Token ?>">
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $SIS_TAREAS_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
</form>
<script type="text/javascript">
