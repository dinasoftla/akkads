...
$SIS_PEDIDO_edit = NULL; // Initialize page object first

class cSIS_PEDIDO_edit extends cSIS_PEDIDO {
	...
	// Page object name
	var $PageObjName = 'SIS_PEDIDO_edit';
	...
	function Page_Main() {
		...
		if ($this->numpedido->CurrentValue == "")
			$this->Page_Terminate("SIS_PEDIDOlist.php"); // Invalid key, return to list
	...
	function ValidateForm() {
		...
		if (in_array("SIS_PEDIDO_DETALLE", $DetailTblVar) && $GLOBALS["SIS_PEDIDO_DETALLE"]->DetailEdit) {
		...
		if ($rs->EOF) {
			...
			if ($bUpdateRow) {
				...
				if ($EditRow) {
					if (in_array("SIS_PEDIDO_DETALLE", $DetailTblVar) && $GLOBALS["SIS_PEDIDO_DETALLE"]->DetailEdit) {
						if (!isset($GLOBALS["SIS_PEDIDO_DETALLE_grid"])) $GLOBALS["SIS_PEDIDO_DETALLE_grid"] = new cSIS_PEDIDO_DETALLE_grid(); // Get detail page object
						$EditRow = $GLOBALS["SIS_PEDIDO_DETALLE_grid"]->GridUpdate();
					}
				}
	...
	function SetUpDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("SIS_PEDIDO_DETALLE", $DetailTblVar)) {
				...
				if ($GLOBALS["SIS_PEDIDO_DETALLE_grid"]->DetailEdit) {
					$GLOBALS["SIS_PEDIDO_DETALLE_grid"]->CurrentMode = "edit";
					$GLOBALS["SIS_PEDIDO_DETALLE_grid"]->CurrentAction = "gridedit";
...
// Create page object
if (!isset($SIS_PEDIDO_edit)) $SIS_PEDIDO_edit = new cSIS_PEDIDO_edit();

// Page init
$SIS_PEDIDO_edit->Page_Init();

// Page main
$SIS_PEDIDO_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$SIS_PEDIDO_edit->Page_Render();
?>
...
var CurrentForm = fSIS_PEDIDOedit = new ew_Form("fSIS_PEDIDOedit", "edit");

// Validate form
fSIS_PEDIDOedit.Validate = function() {
...
fSIS_PEDIDOedit.Form_CustomValidate = 
...
fSIS_PEDIDOedit.ValidateRequired = true;
<?php } else { ?>
fSIS_PEDIDOedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fSIS_PEDIDOedit.Lists["x_codcliente"] = {"LinkField":"x_codcliente","Ajax":true,"AutoFill":false,"DisplayFields":["x_nombre","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fSIS_PEDIDOedit.Lists["x_descuento"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fSIS_PEDIDOedit.Lists["x_descuento"].Options = <?php echo json_encode($SIS_PEDIDO->descuento->Options()) ?>;
...
<?php $SIS_PEDIDO_edit->ShowPageHeader(); ?>
<?php
$SIS_PEDIDO_edit->ShowMessage();
?>
<form name="fSIS_PEDIDOedit" id="fSIS_PEDIDOedit" class="<?php echo $SIS_PEDIDO_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($SIS_PEDIDO_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $SIS_PEDIDO_edit->Token ?>">
<?php } ?>
...
<?php if ($SIS_PEDIDO->numpedido->Visible) { // numpedido ?>
	<div id="r_numpedido" class="form-group">
		<label id="elh_SIS_PEDIDO_numpedido" class="col-sm-2 control-label ewLabel"><?php echo $SIS_PEDIDO->numpedido->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $SIS_PEDIDO->numpedido->CellAttributes() ?>>
<span id="el_SIS_PEDIDO_numpedido">
<span<?php echo $SIS_PEDIDO->numpedido->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $SIS_PEDIDO->numpedido->EditValue ?></p></span>
</span>
<input type="hidden" data-table="SIS_PEDIDO" data-field="x_numpedido" name="x_numpedido" id="x_numpedido" value="<?php echo ew_HtmlEncode($SIS_PEDIDO->numpedido->CurrentValue) ?>">
<?php echo $SIS_PEDIDO->numpedido->CustomMsg ?></div></div>
	</div>
<?php } ?>
...
<?php
	if (in_array("SIS_PEDIDO_DETALLE", explode(",", $SIS_PEDIDO->getCurrentDetailTable())) && $SIS_PEDIDO_DETALLE->DetailEdit) {
?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $SIS_PEDIDO_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
</form>
<script type="text/javascript">
fSIS_PEDIDOedit.Init();
</script>
<?php
$SIS_PEDIDO_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$SIS_PEDIDO_edit->Page_Terminate();
?>
