...
$SIS_KARDEX_view = NULL; // Initialize page object first

class cSIS_KARDEX_view extends cSIS_KARDEX {
	...
	// Page object name
	var $PageObjName = 'SIS_KARDEX_view';
	...
	function __construct() {
		...
		if (@$_GET["fecha"] <> "") {
			$this->RecKey["fecha"] = $_GET["fecha"];
			$KeyUrl .= "&amp;fecha=" . urlencode($this->RecKey["fecha"]);
		}
		if (@$_GET["codarticulo"] <> "") {
	...
	function Page_Init() {
		...
		if (@$_GET["fecha"] <> "") {
			if ($gsExportFile <> "") $gsExportFile .= "_";
			$gsExportFile .= ew_StripSlashes($_GET["fecha"]);
		}
		if (@$_GET["codarticulo"] <> "") {
			if ($gsExportFile <> "") $gsExportFile .= "_";
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["consecutivo"] <> "") {
				...
				$sReturnUrl = "SIS_KARDEXlist.php"; // Return to list
			}
			if (@$_GET["fecha"] <> "") {
				$this->fecha->setQueryStringValue($_GET["fecha"]);
				$this->RecKey["fecha"] = $this->fecha->QueryStringValue;
			} elseif (@$_POST["fecha"] <> "") {
				$this->fecha->setFormValue($_POST["fecha"]);
				$this->RecKey["fecha"] = $this->fecha->FormValue;
			} else {
				$sReturnUrl = "SIS_KARDEXlist.php"; // Return to list
			}
			if (@$_GET["codarticulo"] <> "") {
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "SIS_KARDEXlist.php"; // No matching record, return to list
					}
			}
			...
			$sReturnUrl = "SIS_KARDEXlist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
	...
	function SetupExportOptions() {
		...
		$url = "";
		$item->Body = "<button id=\"emf_SIS_KARDEX\" class=\"ewExportLink ewEmail\" title=\"" . $Language->Phrase("ExportToEmailText") . "\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_SIS_KARDEX',hdr:ewLanguage.Phrase('ExportToEmailText'),f:document.fSIS_KARDEXview,key:" . ew_ArrayToJsonAttr($this->RecKey) . ",sel:false" . $url . "});\">" . $Language->Phrase("ExportToEmail") . "</button>";
		$item->Visible = FALSE;
...
// Create page object
if (!isset($SIS_KARDEX_view)) $SIS_KARDEX_view = new cSIS_KARDEX_view();

// Page init
$SIS_KARDEX_view->Page_Init();

// Page main
$SIS_KARDEX_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$SIS_KARDEX_view->Page_Render();
?>
<?php include_once "header.php" ?>
<?php if ($SIS_KARDEX->Export == "") { ?>
<script type="text/javascript">

// Form object
var CurrentPageID = EW_PAGE_ID = "view";
var CurrentForm = fSIS_KARDEXview = new ew_Form("fSIS_KARDEXview", "view");

// Form_CustomValidate event
fSIS_KARDEXview.Form_CustomValidate = 
...
fSIS_KARDEXview.ValidateRequired = true;
<?php } else { ?>
fSIS_KARDEXview.ValidateRequired = false; 
<?php } ?>
...
<?php } ?>
<?php $SIS_KARDEX_view->ExportOptions->Render("body") ?>
<?php
	foreach ($SIS_KARDEX_view->OtherOptions as &$option)
		$option->Render("body");
?>
<?php if ($SIS_KARDEX->Export == "") { ?>
...
<?php } ?>
<?php $SIS_KARDEX_view->ShowPageHeader(); ?>
<?php
$SIS_KARDEX_view->ShowMessage();
?>
<form name="fSIS_KARDEXview" id="fSIS_KARDEXview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($SIS_KARDEX_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $SIS_KARDEX_view->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="SIS_KARDEX">
<table class="table table-bordered table-striped ewViewTable">
<?php if ($SIS_KARDEX->consecutivo->Visible) { // consecutivo ?>
	<tr id="r_consecutivo">
		<td><span id="elh_SIS_KARDEX_consecutivo"><?php echo $SIS_KARDEX->consecutivo->FldCaption() ?></span></td>
...
<?php echo $SIS_KARDEX->consecutivo->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($SIS_KARDEX->fecha->Visible) { // fecha ?>
	<tr id="r_fecha">
		<td><span id="elh_SIS_KARDEX_fecha"><?php echo $SIS_KARDEX->fecha->FldCaption() ?></span></td>
...
<?php echo $SIS_KARDEX->fecha->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($SIS_KARDEX->numfactura->Visible) { // numfactura ?>
	<tr id="r_numfactura">
		<td><span id="elh_SIS_KARDEX_numfactura"><?php echo $SIS_KARDEX->numfactura->FldCaption() ?></span></td>
...
<?php echo $SIS_KARDEX->numfactura->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($SIS_KARDEX->codarticulo->Visible) { // codarticulo ?>
	<tr id="r_codarticulo">
		<td><span id="elh_SIS_KARDEX_codarticulo"><?php echo $SIS_KARDEX->codarticulo->FldCaption() ?></span></td>
...
<?php echo $SIS_KARDEX->codarticulo->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($SIS_KARDEX->movimiento->Visible) { // movimiento ?>
	<tr id="r_movimiento">
		<td><span id="elh_SIS_KARDEX_movimiento"><?php echo $SIS_KARDEX->movimiento->FldCaption() ?></span></td>
...
<?php echo $SIS_KARDEX->movimiento->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($SIS_KARDEX->cantidad->Visible) { // cantidad ?>
	<tr id="r_cantidad">
		<td><span id="elh_SIS_KARDEX_cantidad"><?php echo $SIS_KARDEX->cantidad->FldCaption() ?></span></td>
...
<?php echo $SIS_KARDEX->cantidad->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($SIS_KARDEX->saldo->Visible) { // saldo ?>
	<tr id="r_saldo">
		<td><span id="elh_SIS_KARDEX_saldo"><?php echo $SIS_KARDEX->saldo->FldCaption() ?></span></td>
...
<?php echo $SIS_KARDEX->saldo->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($SIS_KARDEX->Descripcion->Visible) { // Descripcion ?>
	<tr id="r_Descripcion">
		<td><span id="elh_SIS_KARDEX_Descripcion"><?php echo $SIS_KARDEX->Descripcion->FldCaption() ?></span></td>
...
<?php echo $SIS_KARDEX->Descripcion->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($SIS_KARDEX->usuarioultimamod->Visible) { // usuarioultimamod ?>
	<tr id="r_usuarioultimamod">
		<td><span id="elh_SIS_KARDEX_usuarioultimamod"><?php echo $SIS_KARDEX->usuarioultimamod->FldCaption() ?></span></td>
...
<?php echo $SIS_KARDEX->usuarioultimamod->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php if ($SIS_KARDEX->Export == "") { ?>
<script type="text/javascript">
fSIS_KARDEXview.Init();
</script>
<?php } ?>
<?php
$SIS_KARDEX_view->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($SIS_KARDEX->Export == "") { ?>
...
<?php
$SIS_KARDEX_view->Page_Terminate();
?>
