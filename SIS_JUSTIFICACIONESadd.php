...
$SIS_JUSTIFICACIONES_add = NULL; // Initialize page object first

class cSIS_JUSTIFICACIONES_add extends cSIS_JUSTIFICACIONES {
	...
	// Page object name
	var $PageObjName = 'SIS_JUSTIFICACIONES_add';
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["idjustificacion"] != "") {
				...
				$this->setKey("idjustificacion", $this->idjustificacion->CurrentValue); // Set up key
			} else {
				$this->setKey("idjustificacion", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "SIS_JUSTIFICACIONESview.php")
	...
	function AddRow($rsold = NULL) {
		...
		if ($bInsertRow) {
			...
			if ($AddRow) {
				...
				$this->idjustificacion->setDbValue($conn->Insert_ID());
				$rsnew['idjustificacion'] = $this->idjustificacion->DbValue;
			}
		} else {
...
// Create page object
if (!isset($SIS_JUSTIFICACIONES_add)) $SIS_JUSTIFICACIONES_add = new cSIS_JUSTIFICACIONES_add();

// Page init
$SIS_JUSTIFICACIONES_add->Page_Init();

// Page main
$SIS_JUSTIFICACIONES_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$SIS_JUSTIFICACIONES_add->Page_Render();
?>
...
var CurrentForm = fSIS_JUSTIFICACIONESadd = new ew_Form("fSIS_JUSTIFICACIONESadd", "add");

// Validate form
fSIS_JUSTIFICACIONESadd.Validate = function() {
...
fSIS_JUSTIFICACIONESadd.Form_CustomValidate = 
...
fSIS_JUSTIFICACIONESadd.ValidateRequired = true;
<?php } else { ?>
fSIS_JUSTIFICACIONESadd.ValidateRequired = false; 
<?php } ?>
...
<?php $SIS_JUSTIFICACIONES_add->ShowPageHeader(); ?>
<?php
$SIS_JUSTIFICACIONES_add->ShowMessage();
?>
<form name="fSIS_JUSTIFICACIONESadd" id="fSIS_JUSTIFICACIONESadd" class="<?php echo $SIS_JUSTIFICACIONES_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($SIS_JUSTIFICACIONES_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $SIS_JUSTIFICACIONES_add->Token ?>">
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $SIS_JUSTIFICACIONES_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
</form>
<script type="text/javascript">
fSIS_JUSTIFICACIONESadd.Init();
</script>
<?php
$SIS_JUSTIFICACIONES_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$SIS_JUSTIFICACIONES_add->Page_Terminate();
?>
