...
$VIW_ENTRADA_INVENTARIO = NULL;

//
// Table class for VIW_ENTRADA_INVENTARIO
//
class cVIW_ENTRADA_INVENTARIO extends cTable {
	var $codarticulo;
	var $descripcion;
	var $canarticulo;
	var $canapartado;
	var $candisponible;
	var $CantidadSaliente;
	...
	function __construct() {
		...
		$this->TableVar = 'VIW_ENTRADA_INVENTARIO';
		$this->TableName = 'VIW_ENTRADA_INVENTARIO';
		$this->TableType = 'VIEW';

		// Update Table
		$this->UpdateTable = "[dbo].[VIW_ENTRADA_INVENTARIO]";
		$this->DBID = 'DB';
		...
		// codarticulo
		$this->codarticulo = new cField('VIW_ENTRADA_INVENTARIO', 'VIW_ENTRADA_INVENTARIO', 'x_codarticulo', 'codarticulo', '[codarticulo]', '[codarticulo]', 200, -1, FALSE, '[codarticulo]', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['codarticulo'] = &$this->codarticulo;

		// descripcion
		$this->descripcion = new cField('VIW_ENTRADA_INVENTARIO', 'VIW_ENTRADA_INVENTARIO', 'x_descripcion', 'descripcion', '[descripcion]', '[descripcion]', 200, -1, FALSE, '[descripcion]', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['descripcion'] = &$this->descripcion;

		// canarticulo
		$this->canarticulo = new cField('VIW_ENTRADA_INVENTARIO', 'VIW_ENTRADA_INVENTARIO', 'x_canarticulo', 'canarticulo', '[canarticulo]', 'CAST([canarticulo] AS NVARCHAR)', 3, -1, FALSE, '[canarticulo]', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->canarticulo->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['canarticulo'] = &$this->canarticulo;

		// canapartado
		$this->canapartado = new cField('VIW_ENTRADA_INVENTARIO', 'VIW_ENTRADA_INVENTARIO', 'x_canapartado', 'canapartado', '[canapartado]', 'CAST([canapartado] AS NVARCHAR)', 3, -1, FALSE, '[canapartado]', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->canapartado->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['canapartado'] = &$this->canapartado;

		// candisponible
		$this->candisponible = new cField('VIW_ENTRADA_INVENTARIO', 'VIW_ENTRADA_INVENTARIO', 'x_candisponible', 'candisponible', '[candisponible]', 'CAST([candisponible] AS NVARCHAR)', 3, -1, FALSE, '[candisponible]', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->candisponible->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['candisponible'] = &$this->candisponible;

		// CantidadSaliente
		$this->CantidadSaliente = new cField('VIW_ENTRADA_INVENTARIO', 'VIW_ENTRADA_INVENTARIO', 'x_CantidadSaliente', 'CantidadSaliente', '[CantidadSaliente]', '[CantidadSaliente]', 200, -1, FALSE, '[CantidadSaliente]', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['CantidadSaliente'] = &$this->CantidadSaliente;
	}
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "[dbo].[VIW_ENTRADA_INVENTARIO]";
	}
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "VIW_ENTRADA_INVENTARIOlist.php";
		}
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("VIW_ENTRADA_INVENTARIOview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("VIW_ENTRADA_INVENTARIOview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "VIW_ENTRADA_INVENTARIOadd.php?" . $this->UrlParm($parm);
		else
			$url = "VIW_ENTRADA_INVENTARIOadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("VIW_ENTRADA_INVENTARIOedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("VIW_ENTRADA_INVENTARIOadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("VIW_ENTRADA_INVENTARIOdelete.php", $this->UrlParm());
	}
   ...
   // Common render codes
	...
	function RenderEditRow() {
		...
		// CantidadSaliente
		$this->CantidadSaliente->EditAttrs["class"] = "form-control";
		$this->CantidadSaliente->EditCustomAttributes = "";
		$this->CantidadSaliente->EditValue = $this->CantidadSaliente->CurrentValue;
		$this->CantidadSaliente->PlaceHolder = ew_RemoveHtml($this->CantidadSaliente->FldCaption());
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->CantidadSaliente->Exportable) $Doc->ExportCaption($this->CantidadSaliente);
				} else {
					if ($this->codarticulo->Exportable) $Doc->ExportCaption($this->codarticulo);
					if ($this->descripcion->Exportable) $Doc->ExportCaption($this->descripcion);
					if ($this->canarticulo->Exportable) $Doc->ExportCaption($this->canarticulo);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->CantidadSaliente->Exportable) $Doc->ExportField($this->CantidadSaliente);
					} else {
						if ($this->codarticulo->Exportable) $Doc->ExportField($this->codarticulo);
						if ($this->descripcion->Exportable) $Doc->ExportField($this->descripcion);
						if ($this->canarticulo->Exportable) $Doc->ExportField($this->canarticulo);
