...
$SIS_FACTURA_DETALLE_add = NULL; // Initialize page object first

class cSIS_FACTURA_DETALLE_add extends cSIS_FACTURA_DETALLE {
	...
	// Page object name
	var $PageObjName = 'SIS_FACTURA_DETALLE_add';
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["numdetfactura"] != "") {
				...
				$this->setKey("numdetfactura", $this->numdetfactura->CurrentValue); // Set up key
			} else {
				$this->setKey("numdetfactura", ""); // Clear key
				$this->CopyRecord = FALSE;
			}
			if ($this->CopyRecord) {
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "SIS_FACTURA_DETALLEview.php")
	...
	function RenderRow() {
		...
		$this->total->ViewCustomAttributes = "";
			...
			if ($this->numfactura->getSessionValue() <> "") {
				$this->numfactura->CurrentValue = $this->numfactura->getSessionValue();
			$this->numfactura->ViewValue = $this->numfactura->CurrentValue;
			$this->numfactura->ViewCustomAttributes = "";
			} else {
	...
	function AddRow($rsold = NULL) {
		...
		if (strval($this->numfactura->CurrentValue) <> "") {
			$sMasterFilter = str_replace("@numfactura@", ew_AdjustSql($this->numfactura->CurrentValue, "DB"), $sMasterFilter);
		} else {
		...
		// numfactura
		$this->numfactura->SetDbValueDef($rsnew, $this->numfactura->CurrentValue, 0, FALSE);
		...
		if ($bInsertRow) {
			...
			if ($AddRow) {
				...
				$this->numdetfactura->setDbValue($conn->Insert_ID());
				$rsnew['numdetfactura'] = $this->numdetfactura->DbValue;
			}
		} else {
...
// Create page object
if (!isset($SIS_FACTURA_DETALLE_add)) $SIS_FACTURA_DETALLE_add = new cSIS_FACTURA_DETALLE_add();

// Page init
$SIS_FACTURA_DETALLE_add->Page_Init();

// Page main
$SIS_FACTURA_DETALLE_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$SIS_FACTURA_DETALLE_add->Page_Render();
?>
...
var CurrentForm = fSIS_FACTURA_DETALLEadd = new ew_Form("fSIS_FACTURA_DETALLEadd", "add");

// Validate form
fSIS_FACTURA_DETALLEadd.Validate = function() {
...
fSIS_FACTURA_DETALLEadd.Form_CustomValidate = 
...
fSIS_FACTURA_DETALLEadd.ValidateRequired = true;
<?php } else { ?>
fSIS_FACTURA_DETALLEadd.ValidateRequired = false; 
<?php } ?>
...
<?php $SIS_FACTURA_DETALLE_add->ShowPageHeader(); ?>
<?php
$SIS_FACTURA_DETALLE_add->ShowMessage();
?>
<form name="fSIS_FACTURA_DETALLEadd" id="fSIS_FACTURA_DETALLEadd" class="<?php echo $SIS_FACTURA_DETALLE_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($SIS_FACTURA_DETALLE_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $SIS_FACTURA_DETALLE_add->Token ?>">
<?php } ?>
...
<input type="hidden" id="x_numfactura" name="x_numfactura" value="<?php echo ew_HtmlEncode($SIS_FACTURA_DETALLE->numfactura->CurrentValue) ?>">
<?php } else { ?>
<span id="el_SIS_FACTURA_DETALLE_numfactura">
<input type="text" data-table="SIS_FACTURA_DETALLE" data-field="x_numfactura" name="x_numfactura" id="x_numfactura" size="30" placeholder="<?php echo ew_HtmlEncode($SIS_FACTURA_DETALLE->numfactura->getPlaceHolder()) ?>" value="<?php echo $SIS_FACTURA_DETALLE->numfactura->EditValue ?>"<?php echo $SIS_FACTURA_DETALLE->numfactura->EditAttributes() ?>>
</span>
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $SIS_FACTURA_DETALLE_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
</form>
<script type="text/javascript">
fSIS_FACTURA_DETALLEadd.Init();
</script>
<?php
$SIS_FACTURA_DETALLE_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$SIS_FACTURA_DETALLE_add->Page_Terminate();
?>
