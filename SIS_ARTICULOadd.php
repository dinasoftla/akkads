...
$SIS_ARTICULO_add = NULL; // Initialize page object first

class cSIS_ARTICULO_add extends cSIS_ARTICULO {
	...
	// Page object name
	var $PageObjName = 'SIS_ARTICULO_add';
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "SIS_ARTICULOview.php")
	...
	function LoadDefaultValues() {
		...
		$this->foto3->Upload->DbValue = NULL;
		$this->foto3->OldValue = $this->foto3->Upload->DbValue;
		$this->foto3->CurrentValue = NULL; // Clear file related field
		$this->caracteristicas->CurrentValue = NULL;
		$this->caracteristicas->OldValue = $this->caracteristicas->CurrentValue;
		$this->garantia->CurrentValue = NULL;
		$this->garantia->OldValue = $this->garantia->CurrentValue;
		$this->referencia->CurrentValue = NULL;
		$this->referencia->OldValue = $this->referencia->CurrentValue;
		$this->codlinea->CurrentValue = NULL;
		$this->codlinea->OldValue = $this->codlinea->CurrentValue;
		$this->calificacion->CurrentValue = NULL;
		$this->calificacion->OldValue = $this->calificacion->CurrentValue;
	...
	function RenderRow() {
		...
		$this->usuarioultimamod->ViewCustomAttributes = "";
			...
			if (($this->CurrentAction == "I" || $this->CurrentAction == "C") && !$this->EventCancelled) ew_RenderUploadField($this->foto3);
	...
	function AddRow($rsold = NULL) {
		...
		if ($this->foto2->Visible && !$this->foto2->Upload->KeepFile) {
			$this->foto2->Upload->DbValue = ""; // No need to delete old file
		...
		if ($this->foto3->Visible && !$this->foto3->Upload->KeepFile) {
			$this->foto3->Upload->DbValue = ""; // No need to delete old file
		...
		// caracteristicas
		$this->caracteristicas->SetDbValueDef($rsnew, $this->caracteristicas->CurrentValue, "", FALSE);

		// garantia
		$this->garantia->SetDbValueDef($rsnew, $this->garantia->CurrentValue, NULL, FALSE);

		// referencia
		$this->referencia->SetDbValueDef($rsnew, $this->referencia->CurrentValue, "", FALSE);

		// codlinea
		$this->codlinea->SetDbValueDef($rsnew, $this->codlinea->CurrentValue, 0, FALSE);

		// calificacion
		$this->calificacion->SetDbValueDef($rsnew, $this->calificacion->CurrentValue, NULL, FALSE);
...
// Create page object
if (!isset($SIS_ARTICULO_add)) $SIS_ARTICULO_add = new cSIS_ARTICULO_add();

// Page init
$SIS_ARTICULO_add->Page_Init();

// Page main
$SIS_ARTICULO_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$SIS_ARTICULO_add->Page_Render();
?>
...
var CurrentForm = fSIS_ARTICULOadd = new ew_Form("fSIS_ARTICULOadd", "add");

// Validate form
fSIS_ARTICULOadd.Validate = function() {
...
fSIS_ARTICULOadd.Form_CustomValidate = 
...
fSIS_ARTICULOadd.ValidateRequired = true;
<?php } else { ?>
fSIS_ARTICULOadd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fSIS_ARTICULOadd.Lists["x_codlinea"] = {"LinkField":"x_cod_linea","Ajax":true,"AutoFill":false,"DisplayFields":["x_Descripcion","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fSIS_ARTICULOadd.Lists["x_calificacion"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fSIS_ARTICULOadd.Lists["x_calificacion"].Options = <?php echo json_encode($SIS_ARTICULO->calificacion->Options()) ?>;
...
<?php $SIS_ARTICULO_add->ShowPageHeader(); ?>
<?php
$SIS_ARTICULO_add->ShowMessage();
?>
<form name="fSIS_ARTICULOadd" id="fSIS_ARTICULOadd" class="<?php echo $SIS_ARTICULO_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($SIS_ARTICULO_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $SIS_ARTICULO_add->Token ?>">
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $SIS_ARTICULO_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
</form>
<script type="text/javascript">
fSIS_ARTICULOadd.Init();
</script>
<?php
$SIS_ARTICULO_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$SIS_ARTICULO_add->Page_Terminate();
?>
