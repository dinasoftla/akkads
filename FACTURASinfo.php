...
class cFACTURAS extends cTable {
	...
	function __construct() {
		...
		// numfactura
		$this->numfactura = new cField('FACTURAS', 'FACTURAS', 'x_numfactura', 'numfactura', '[numfactura]', 'CAST([numfactura] AS NVARCHAR)', 3, -1, FALSE, '[numfactura]', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->numfactura->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['numfactura'] = &$this->numfactura;

		// fecha
		$this->fecha = new cField('FACTURAS', 'FACTURAS', 'x_fecha', 'fecha', '[fecha]', '[fecha]', 202, -1, FALSE, '[fecha]', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['fecha'] = &$this->fecha;

		// codcliente
		$this->codcliente = new cField('FACTURAS', 'FACTURAS', 'x_codcliente', 'codcliente', '[codcliente]', 'CAST([codcliente] AS NVARCHAR)', 3, -1, FALSE, '[codcliente]', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->codcliente->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['codcliente'] = &$this->codcliente;

		// descuento
		$this->descuento = new cField('FACTURAS', 'FACTURAS', 'x_descuento', 'descuento', '[descuento]', 'CAST([descuento] AS NVARCHAR)', 5, -1, FALSE, '[descuento]', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->descuento->FldDefaultErrMsg = $Language->Phrase("IncorrectFloat");
		$this->fields['descuento'] = &$this->descuento;

		// estado
		$this->estado = new cField('FACTURAS', 'FACTURAS', 'x_estado', 'estado', '[estado]', '[estado]', 200, -1, FALSE, '[estado]', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "[dbo].[FACTURAS]";
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("FACTURASview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("FACTURASview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "FACTURASadd.php?" . $this->UrlParm($parm);
		else
			$url = "FACTURASadd.php";
		return $this->AddMasterUrl($url);
	}
	...
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("FACTURASedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("FACTURASadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
   ...
   // Common render codes
	...
	function RenderEditRow() {
		...
		$this->codcliente->PlaceHolder = ew_RemoveHtml($this->codcliente->FldCaption());
